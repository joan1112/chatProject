...
+ (NSURLSessionTask *)GET:(NSString *)URL
    ...
    NSString *url = [NSString stringWithFormat:@"%@%@",mainServerURL,URL];
...
//            [vc.view hideHUD];
            ...
            [window showToastWithText:@"网络不给力，请稍后重试"];
//            [vc showNoInternetView];
    ...
    NSURLSessionTask *sessionTask =[_sessionManager POST:url parameters:parameters headers:@{} progress:^(NSProgress * _Nonnull uploadProgress) {
        ...
        NSLog(@"------responseSUCCESS------%@",responseObject);
        if (_isOpenLog) {NSLog(@"responseObject = %@",responseObject);}
        [[self allSessionTask] removeObject:task];
        success ? success(responseObject) : nil;
        
    } failure:^(NSURLSessionDataTask * _Nullable task, NSError * _Nonnull error) {
        NSLog(@"上传错误至emm ------responseERROR------%@",error);
...
//            NSString *path =[NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES)firstObject];
        ...
        NSString *downloadDir = [[NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES) lastObject] stringByAppendingPathComponent:fileDir ? fileDir : @"pathToBundle"];
        //打开文件管理器
