...
#import "ChatManger.h"
@interface ChatView()<NIMMediaManagerDelegate,NIMInputDelegate>
{
    UIImageView *content;
    UILabel *labTitle;
}
...
@property (nonatomic,strong)    ChatManger *configurator;
...
-(void)creatView
{
    ...
    labTitle = [[UILabel alloc]initWithFrame:CGRectMake(10, 10, self.width-20, 30)];
    labTitle.text = [NIMKitUtil showNick:self.session.sessionId inSession:self.session];;
    [top addSubview:labTitle];
    labTitle.textAlignment = NSTextAlignmentCenter;
    labTitle.textColor = UIColor.whiteColor;
    labTitle.font = [UIFont systemFontOfSize:14];
    
    content = [[UIImageView alloc]initWithFrame:CGRectMake(0, CGRectGetMaxY(top.frame), self.width, self.height-50)];
    content.image = [UIImage imageNamed:@"friend_c_bg"];
    [self addSubview:content];
    content.userInteractionEnabled = YES;
}

- (instancetype)initWithSession:(NIMSession *)session WithFrame:(CGRect)frame{
    self = [super initWithFrame:frame];
    if (self) {
        _session = session;
...
//{
//    if ([self.delegate respondsToSelector:@selector(dismissChatView)]) {
//        [self.delegate dismissChatView];
//    }
//}
- (void)setupNav
{
...
- (void)setupTableView
{
    ...
    self.tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 0, content.width, content.height) style:UITableViewStylePlain];
...
- (void)setupInputView
{
    ...
    {
        self.sessionInputView = [[NIMInputView alloc] initWithFrame:CGRectMake(0, 0, self.nim_width,0) config:self.sessionConfig];
        ...
        [self addSubview:_sessionInputView];
    }
}
...
{
    _configurator = [[ChatManger alloc] init];
...
//                [self.view hideToasts];
                if (error)
                {
                    [weakSelf makeToast:@"操作失败".nim_localized duration:2 position:CSToastPositionCenter];
                }
